"use client";

import { useState, useTransition, useMemo } from "react";
import { ColumnDef } from "@tanstack/react-table";
import {
  Truck,
  AlertTriangle,
  CheckCircle2,
  Wrench,
  PowerOff,
  RefreshCw,
  Loader2,
} from "lucide-react";
import { ModalEditarUnidad } from "./modal-editar-unidad";
import { cambiarEstadoUnidadAction, darDeBajaUnidadAction } from "@/lib/actions/flota";
import { toast } from "sonner";
import { DataTable, DataTableColumnHeader } from "@/components/ui/data-table";

interface UnidadItem {
  id: string;
  placa: string;
  tipoUnidad: "tracto" | "rigido" | "camioneta";
  marca: string;
  modelo: string;
  anioFabricacion: number;
  color?: string | null;
  vinChasis?: string | null;
  numeroMotor?: string | null;
  ejes: number;
  capacidadArrastreTn?: string | null;
  pesoSecoTn?: string | null;
  tipoCombustible: "diesel_b5" | "gnv" | "glp";
  odometroActualKm: number;
  idDispositivoGps?: string | null;
  estado: "disponible" | "en_ruta" | "mantenimiento" | "inactivo";
  activo: boolean;
  acoplamientos?: Array<{
    id: string;
    semirremolqueId: string;
    activo: boolean;
  }>;
}

interface SemirremolqueRef {
  id: string;
  placa: string;
  tipoCarroceria: string;
}

interface DocumentoRef {
  id: string;
  entidadTipo: string;
  entidadId: string;
  estadoAlerta: string;
}

interface TablaUnidadesProps {
  unidades: UnidadItem[];
  semirremolques: SemirremolqueRef[];
  documentos: DocumentoRef[];
}

export function TablaUnidades({ unidades, semirremolques, documentos }: TablaUnidadesProps) {
  const [filtroEstado, setFiltroEstado] = useState<string>("todos");
  const [isPending, startTransition] = useTransition();

  const getDocumentosInfo = (unidadId: string) => {
    const docs = documentos.filter(
      (d) => d.entidadTipo === "unidad" && d.entidadId === unidadId
    );
    const hasExpired = docs.some((d) => d.estadoAlerta === "vencido");
    const hasWarning = docs.some((d) => d.estadoAlerta === "por_vencer");

    if (hasExpired) {
      return { label: "Doc. Vencido", variant: "destructive" as const };
    }
    if (hasWarning) {
      return { label: "Por Vencer", variant: "warning" as const };
    }
    return { label: "Al día", variant: "success" as const };
  };

  const handleToggleMantenimiento = (u: UnidadItem) => {
    const nuevoEstado = u.estado === "mantenimiento" ? "disponible" : "mantenimiento";
    startTransition(async () => {
      const res = await cambiarEstadoUnidadAction(u.id, nuevoEstado);
      if (res.success) {
        toast.success(
          `Unidad ${u.placa} ${
            nuevoEstado === "mantenimiento" ? "enviada a taller" : "marcada como disponible"
          }.`
        );
      } else {
        toast.error(res.error || "No se pudo cambiar el estado.");
      }
    });
  };

  const handleBaja = (u: UnidadItem) => {
    if (
      !confirm(
        `¿Seguro que deseas dar de baja la unidad ${u.placa}? Si tiene acople activo, se desacoplará automáticamente.`
      )
    ) {
      return;
    }
    startTransition(async () => {
      const res = await darDeBajaUnidadAction(u.id);
      if (res.success) {
        toast.success(`Unidad ${u.placa} dada de baja exitosamente.`);
      } else {
        toast.error(res.error || "No se pudo dar de baja la unidad.");
      }
    });
  };

  const filteredData = useMemo(() => {
    if (filtroEstado === "todos") return unidades;
    if (filtroEstado === "activos") {
      return unidades.filter((u) => u.activo && u.estado !== "inactivo");
    }
    return unidades.filter((u) => u.estado === filtroEstado);
  }, [unidades, filtroEstado]);

  const columns = useMemo<ColumnDef<UnidadItem>[]>(
    () => [
      {
        accessorKey: "placa",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Placa MTC" />
        ),
        cell: ({ row }) => (
          <span className="font-mono font-bold text-white text-sm">
            {row.original.placa}
          </span>
        ),
      },
      {
        id: "marcaModelo",
        accessorKey: "marca",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Marca & Modelo" />
        ),
        cell: ({ row }) => {
          const u = row.original;
          return (
            <div>
              <span className="font-semibold text-slate-200 block">{u.marca}</span>
              <span className="text-[11px] text-slate-400">{u.modelo}</span>
            </div>
          );
        },
      },
      {
        accessorKey: "anioFabricacion",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Año / Ejes" />
        ),
        cell: ({ row }) => {
          const u = row.original;
          return (
            <span className="text-slate-300">
              {u.anioFabricacion} · <span className="font-mono">{u.ejes} ejes</span>
            </span>
          );
        },
      },
      {
        accessorKey: "capacidadArrastreTn",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Cap. Arrastre" />
        ),
        sortingFn: (a, b) => {
          const valA = parseFloat(a.original.capacidadArrastreTn || "0");
          const valB = parseFloat(b.original.capacidadArrastreTn || "0");
          return valA - valB;
        },
        cell: ({ row }) => (
          <span className="font-mono font-medium text-slate-200">
            {row.original.capacidadArrastreTn || 0} Tn
          </span>
        ),
      },
      {
        accessorKey: "odometroActualKm",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Odómetro Actual" />
        ),
        cell: ({ row }) => (
          <span className="font-mono text-slate-300">
            {row.original.odometroActualKm.toLocaleString("es-PE")} km
          </span>
        ),
      },
      {
        id: "semirremolque",
        header: "Semirremolque",
        cell: ({ row }) => {
          const u = row.original;
          const acopleActivo = u.acoplamientos?.[0];
          const carreta = acopleActivo
            ? semirremolques.find((s) => s.id === acopleActivo.semirremolqueId)
            : null;

          if (carreta) {
            return (
              <div className="flex items-center gap-1.5">
                <span className="font-mono font-bold text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded text-[11px] border border-amber-400/20">
                  {carreta.placa}
                </span>
                <span className="text-[10px] text-slate-400 capitalize">
                  {carreta.tipoCarroceria.replace("_", " ")}
                </span>
              </div>
            );
          }

          return (
            <span className="text-slate-500 text-[11px] italic">
              Libre (Desacoplado)
            </span>
          );
        },
      },
      {
        id: "vigencias",
        header: "Vigencias MTC",
        cell: ({ row }) => {
          const docInfo = getDocumentosInfo(row.original.id);
          return (
            <div>
              {docInfo.variant === "destructive" ? (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold bg-rose-500/15 text-rose-400 border border-rose-500/30">
                  <AlertTriangle className="h-3 w-3" />
                  {docInfo.label}
                </span>
              ) : docInfo.variant === "warning" ? (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold bg-amber-500/15 text-amber-400 border border-amber-500/30">
                  <AlertTriangle className="h-3 w-3" />
                  {docInfo.label}
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">
                  <CheckCircle2 className="h-3 w-3" />
                  {docInfo.label}
                </span>
              )}
            </div>
          );
        },
      },
      {
        accessorKey: "estado",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Estado" />
        ),
        cell: ({ row }) => {
          const estado = row.original.estado;
          if (estado === "en_ruta") {
            return (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 animate-pulse">
                En Ruta
              </span>
            );
          }
          if (estado === "mantenimiento") {
            return (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase bg-amber-500/15 text-amber-400 border border-amber-500/30">
                En Taller
              </span>
            );
          }
          if (estado === "inactivo") {
            return (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase bg-rose-500/15 text-rose-400 border border-rose-500/30">
                De Baja
              </span>
            );
          }
          return (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase bg-blue-500/15 text-blue-400 border border-blue-500/30">
              Disponible
            </span>
          );
        },
      },
      {
        id: "acciones",
        header: () => <div className="text-right">Acciones</div>,
        cell: ({ row }) => {
          const u = row.original;
          return (
            <div className="flex items-center justify-end gap-1.5">
              <ModalEditarUnidad unidad={u} />
              {u.estado !== "inactivo" && (
                <>
                  <button
                    type="button"
                    onClick={() => handleToggleMantenimiento(u)}
                    disabled={isPending}
                    title={
                      u.estado === "mantenimiento"
                        ? "Retornar a Disponible"
                        : "Enviar a Taller"
                    }
                    className="p-1.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-amber-400 transition-colors disabled:opacity-50"
                  >
                    {u.estado === "mantenimiento" ? (
                      <RefreshCw className="h-3.5 w-3.5 text-blue-400" />
                    ) : (
                      <Wrench className="h-3.5 w-3.5" />
                    )}
                  </button>
                  <button
                    type="button"
                    onClick={() => handleBaja(u)}
                    disabled={isPending}
                    title="Dar de baja unidad"
                    className="p-1.5 rounded bg-slate-800 hover:bg-rose-950/40 text-slate-400 hover:text-rose-400 border border-slate-700 transition-colors disabled:opacity-50"
                  >
                    <PowerOff className="h-3.5 w-3.5" />
                  </button>
                </>
              )}
            </div>
          );
        },
      },
    ],
    [semirremolques, documentos, isPending]
  );

  return (
    <DataTable
      columns={columns}
      data={filteredData}
      searchPlaceholder="Buscar unidad por placa, marca o modelo..."
      initialPageSize={10}
      toolbarLeft={
        <div className="flex items-center gap-2">
          <select
            value={filtroEstado}
            onChange={(e) => setFiltroEstado(e.target.value)}
            className="h-8 bg-[#111827] border border-[#1F2937] rounded px-2 text-xs text-slate-300 focus:outline-none focus:border-amber-500"
          >
            <option value="todos">Todos los Estados</option>
            <option value="disponible">🔵 Disponibles</option>
            <option value="en_ruta">🟢 En Ruta</option>
            <option value="mantenimiento">🟡 En Taller</option>
            <option value="inactivo">🔴 Inactivos / Baja</option>
          </select>

          <span className="text-xs text-slate-500 hidden sm:inline">
            ({filteredData.length} de {unidades.length})
          </span>
        </div>
      }
      emptyTitle="No se encontraron unidades vehiculares"
      emptyDescription="Prueba ajustando los filtros de búsqueda o registra una nueva unidad vehicular."
      emptyIcon={Truck}
    />
  );
}
